...
function loadModule(vm: Vm, filePath: string): CallbackEntry {
  const name = basename(filePath, ".js");
  const source = readFileSync(filePath, "utf-8");
  const stat = readFileSync(filePath, { encoding: null });
...
function createVm(): Vm {
  ...
  vm.exposeFunction("hostNow", () => {
    return Date.now();
  });

  vm.exposeFunction("hostJson", (value: unknown) => {
...
function bootstrapModules(vm: Vm): void {
  ...
  for (const file of sorted) {
    loadModule(vm, join(MODULES_DIR, file));
  }
  console.log("");
}

function runCallbacks(vm: Vm): void {
  console.log("--- Running Callbacks ---\n");

  const greetResult = vm.run(`
    import { greet, farewell, announce } from "greet";
    const r1 = greet("Alice");
    const r2 = farewell("Bob");
    const r3 = announce("Server is starting", "system");
    JSON.stringify([r1, r2, r3]);
  `);
  console.log("greet callbacks:", greetResult);

  const mathResult = vm.run(`
    import { add, multiply, factorial, fib, clampValue } from "math";
    const r1 = add(10, 20);
    const r2 = multiply(6, 7);
    const r3 = factorial(5);
    const r4 = fib(10);
    const r5 = clampValue(150, 0, 100);
    JSON.stringify({ sum: r1, product: r2, fact: r3, fib: r4, clamped: r5 });
  `);
  console.log("math callbacks:", mathResult);

  const transformResult = vm.run(`
    import { capitalize, reverse, repeat, slugify, wordCount } from "transform";
    const r1 = capitalize("hello world");
    const r2 = reverse("abcdef");
    const r3 = repeat("ha", 3);
    const r4 = slugify("Hello World!  --  Foo Bar");
    const r5 = wordCount("  the quick  brown fox  ");
    JSON.stringify({ cap: r1, rev: r2, rep: r3, slug: r4, wc: r5 });
  `);
  console.log("transform callbacks:", transformResult);

  console.log("");
}

function buildCallbackDispatcher(vm: Vm): string {
  return `
    import { greet, farewell, announce } from "greet";
    import { add, multiply, factorial, fib } from "math";
    import { capitalize, reverse, slugify } from "transform";
...
function testDispatch(vm: Vm): void {
  ...
  vm.run(buildCallbackDispatcher(vm));
...
function setupHotReload(vm: Vm): void {
  ...
  const reload = (changedFile: string) => {
    ...
    debounceTimer = setTimeout(() => {
      const filePath = join(MODULES_DIR, changedFile);
      console.log(`[hot-reload] Change detected: ${changedFile}`);

      const entry = loadModule(vm, filePath);
