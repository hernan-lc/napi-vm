...
/** Expose the crypto primitives and register `napi:crypto`. */
export function installCryptoCapability(vm: Vm): void {
...
/** Remove everything `installCryptoCapability` added. */
export function uninstallCryptoCapability(vm: Vm): void {
  vm.removeModule(CRYPTO_MODULE_NAME);
  for (const name of CRYPTO_GLOBALS) vm.removeGlobal(name);
}
